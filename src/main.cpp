...
bool sendHistoricalData()
{
  ...
  while (!myqueue.empty())
  {
    ...
    if (!sendActData(actToSend))
    {                          //if sending data not succesfull -> Connection-Probem, save value and continue in loop
 ...
 * @return true if the date has been sent successfully and 
